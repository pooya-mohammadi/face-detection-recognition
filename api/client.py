...
parser.add_argument("--end_point", default="http://127.0.0.1:8000/face_recognition")
parser.add_argument("--img_address", default='/home/ai/projects/face-detection-recognition/dataset/test/friends.jpg')
args = parser.parse_args()


def infer(end_point, img_address) -> dict:
    image = cv2.imread(img_address)[..., ::-1]
    byte_img = img_to_b64(image)
    data = {'image': byte_img}
    response = requests.post(end_point, json=data, headers={"Content-Type": "application/json"})
...
if __name__ == "__main__":
    output = infer(args.end_point, args.img_address)
